...
class WebPageTest extends PHPUnit_Framework_TestCase
{
    ...
    public function findDataProvider()
    {
        return [
            'script src values' => [
                'response' => ResponseFactory::createFromFixture('Html/document-with-script-elements.html'),
                'selector' => 'script',
            ...
            'script values from charset=gb2313 content' => [
                'response' => ResponseFactory::createFromFixture(
                    'Html/document-with-script-elements-charset=gb2312.html'
                ),
                'selector' => 'script',
                'eachFunction' => function (\DOMElement $domElement) {
                    return trim($domElement->nodeValue);
            ...
            'script values from charset=big5 content' => [
                'response' => ResponseFactory::createFromFixture('Html/document-with-big5-charset.html'),
                'selector' => 'script',
                'eachFunction' => function (\DOMElement $domElement) {
                    return trim($domElement->nodeValue);
                },
    ...
    public function getCharacterSetDataProvider()
    {
        return [
            ...
            'get from document meta when missing in http response' => [
                'response' => ResponseFactory::createFromFixture('Html/empty-document-with-valid-meta-charset.html'),
                'expectedCharacterSet' => 'utf-8',
            ],
            'get when missing in document and http response' => [
                'response' => ResponseFactory::createFromFixture('Html/empty-document.html'),
    ...
    public function getDocumentCharacterSetDataProvider()
    {
        return [
            'meta charset' => [
                'response' => ResponseFactory::createFromFixture(
                    'Html/empty-document-with-valid-meta-charset.html'
                ),
